...
#Fetching the data as Dataframe from SQL table
aggregated_transaction_df = pd.read_sql_table("aggregated_transaction",con=db_engine)
aggregated_user_df = pd.read_sql_table("aggregated_user",con=db_engine)
...
top_user_df = pd.read_sql_table("top_user",con=db_engine)
...
st.title("Phonepe Pulse Data Visualization and Exploration") # project title

st.sidebar.image(r"C:\Users\LENOVO\2.Phonepe puls vizualization\download.jpg",use_column_width=600) #Phonepe Image

st.sidebar.header("Payments")

transaction_parts=["Transactions", "Users"]
...
if Payments == 'Transactions':
    ...
    for i in state:
        ...
        tab4.markdown(f'**Top Pincodes in "{i}_{selected_quater}_{selected_year}"**')
        tab4.dataframe(top_user_pincode_df2[['Postalcodes','Postalcodes_registered_user']],use_container_width=True)
